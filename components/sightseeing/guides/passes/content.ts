 ...
 * 一覧の但し書き。
 *
 * 「当サイトが紹介しているスポットのうち」と明示するのが要点。
 * Go City の対象は110件前後あるが、その多くはウォーキングツアー、
 * 食事つきの体験、eSIM のデータ通信など、施設の入場ではない。
 * ここで件数だけを見せると「110件も入れる」という誤った印象を
 * 補強してしまうので、何を数えた数字なのかを先に言う。
 */
export const covered = {
  ...
  /**
   * 元から無料の館を一覧に出さない理由。読者は必ず「大英博物館は？」と
   * 探すので、無いことの説明を先に置く。
   */
  freeMuseumNote:
  ...
  updatedNote: `対象は入れ替わります。${PASS_AS_OF}時点で確認したものです。買う前に販売ページで最新の対象をご確認ください。`,
} as const;
...
export const passesSections = [
  ...
  { id: "covered", label: "対象施設の一覧", navLabel: "対象一覧" },
