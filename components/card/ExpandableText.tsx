...
import { cn } from "@/lib/utils"; // ← shadcn が提供するクラス結合ヘルパー（重要）

type Props = {
  text: string;
  maxLines?: number; // ← 任意指定、デフォルト3行
  className?: string; // ← 外部クラスを受け取る
};

function ExpandableText({ text, maxLines = 3, className }: Props) {
  return (
    <Collapsible className={cn("space-y-2 group", className)}>
      {/* 最大行数：maxLines に応じて line-clamp を分岐 */}
      <div
        className={cn(
          `
