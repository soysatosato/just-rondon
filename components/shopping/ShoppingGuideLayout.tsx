...
export default function ShoppingGuideLayout({
  article,
}: {
  article: ShoppingGuideArticle;
}) {
  const meta = getShoppingGuideMeta(article.slug);
  const pageUrl = `${SITE_URL}${shoppingGuidePath(article.slug)}`;

  return (
    <main className="mx-auto max-w-4xl px-1 py-10 text-gray-900 dark:text-gray-100 sm:px-4">
      <JsonLd data={shoppingGuideBreadcrumbJsonLd(article)} />
      <JsonLd data={shoppingGuideArticleJsonLd(article)} />
      ...
      {/*
        要約表。買い物の記事は「結局いつ行けばいいのか」だけを知りたい
        読者が多いので、目次より前に曜日と時間を置く。
      */}
      ...
      <section className="mt-12 space-y-6">
        <h2 className="text-lg font-semibold">ほかの買い物ガイド</h2>
        {SHOPPING_CATEGORY_ORDER.map((category) => {
          const guides = shoppingGuidesByCategory(category).filter(
