...
export default function BeyondLayout({ article }: { article: BeyondArticle }) {
  ...
  return (
    <main className="mx-auto max-w-4xl px-1 py-10 text-gray-900 dark:text-gray-100 sm:px-4">
      ...
      {/* 要点表。数字を最初に出すのは transport ガイドと同じ方針。 */}
      ...
      {/*
        行き先記事は「行き方」を本文の直後・目次の前に置く。
        読者が最初に確かめたいのは行き方で、街の紹介はその後でいい。
      */}
      {destination && (
        <BeyondGettingThere
      ...
      */}
      ...
      {/*
        ほかの記事はカテゴリ見出しごと出す。
        「移動の実務」と行き先を混ぜて並べると、BritRail Pass が
        地名だと誤読される。
      */}
      <section className="mt-12 space-y-6">
        <h2 className="text-lg font-semibold">ほかの行き先とガイド</h2>
        {BEYOND_CATEGORY_ORDER.map((category) => {
          ...
          return (
            <div key={category} className="space-y-3">
              ...
              {items.map((d) => (
                <Link key={d.slug} href={beyondPath(d.slug)} className="block">
